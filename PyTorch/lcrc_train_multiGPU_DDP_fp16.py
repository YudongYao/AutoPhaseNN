...
def train_CL(args, model, criterion, trainloader_old, trainloader_new, optimizer, scheduler, epoch, with_DDP, rank, scaler):
    ...
    if args.device == 'cuda':
        loss_ft, loss_amp, loss_ph = loss_ft.cuda(), loss_amp.cuda(), loss_ph.cuda()

    mem_iter = iter(trainloader_old)

    for i, (ft_images, amps, phs) in tqdm(enumerate(trainloader_new)):

        ft_images_m, amps_m, phs_m = next(mem_iter)

        # Transfer to GPU
        if args.device == 'cuda':
            ft_images, amps, phs = ft_images.cuda(), amps.cuda(), phs.cuda()
            ft_images_m, amps_m, phs_m = ft_images_m.cuda(), amps_m.cuda(), phs_m.cuda()
        ...
        with torch.autograd.set_detect_anomaly(True):
            with torch.cuda.amp.autocast(enabled=args.fp16):
                ...
                y_m, _, pred_amps_m, pred_phs_m, support_m = model(
                    ft_images_m)  # Forward pass

                # Compute losses for new data
                loss_f = criterion(y, ft_images)
                loss_a = criterion(pred_amps, amps)  # Monitor amplitude loss
                # Monitor phase loss but only within support (which may not be same as true amp)
                loss_p = criterion(pred_phs*support, phs*support)

                # Compute losses for old data
                loss_f_m = criterion(y_m, ft_images_m)
                loss_a_m = criterion(pred_amps_m, amps_m)  # Monitor amplitude loss
                # Monitor phase loss but only within support (which may not be same as true amp)
                loss_p_m = criterion(pred_phs_m*support_m, phs_m*support_m)
        #
                if args.unsupervise:
                    loss = loss_f + loss_f_m  # Use only FT loss for gradients
                else:
                    loss = loss_a + loss_p + loss_f + loss_a_m + loss_p_m + loss_f_m
            ...
            loss_ft += (loss_f.item() + loss_f_m.item())
            loss_amp += (loss_a.item() + loss_a_m.item())
            loss_ph += (loss_p.item() + loss_p_m.item())
...
def validation_CL(args, model, criterion, validloader_old, validloader_new, optimizer, epoch, with_DDP, rank):
    model.eval()
    ...
    with torch.no_grad():
        for j, (ft_images, amps, phs) in enumerate(validloader):
            ...
            val_loss_ft += (loss_f.item() + loss_f_m.item())
            val_loss_amp += (loss_a.item() + loss_a_m.item())
            val_loss_ph += (loss_p.item() + loss_p_m.item())
